...
parser.add_argument("--contentPath", default="data/content/",
                    help='path to frames')
parser.add_argument("--outf", default="Artistic/default",
...
parser.add_argument('--fineSize', type=int, default=1000,
...
for ci,(content,contentName) in enumerate(content_loader):
    ...
    for sj,(style,styleName) in enumerate(style_loader):
        ...
        #if os.path.exists('%s/%s_%s.png'%(opt.outf,contentName,styleName)):
        #    print('continue: %s' % (contentName))
        #    continue
        ...
        for ii in range(100):
            ...
            try:
                with open('%s/%s_%s.png'%(opt.outf,contentName,styleName), 'rb') as f:
                    img = Image.open(f)
                break
            except:
                print('Image broken -> try again')
                continue
