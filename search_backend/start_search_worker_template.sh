...
then
    ...
    python worker.py localhost 0 1 $API_SECRET $WORKER_NAME ./configs_worker/config_local_$2.json
elif [[ $1 == "server" ]]
then
    # - run on server
    python worker.py $SERVER_ADDRESS 0 1 $API_SECRET $WORKER_NAME ./configs_worker/config_server_$2.json
else
