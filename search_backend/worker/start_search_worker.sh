...
# Start worker
echo "Start search worker"
if [[ $1 == "server" ]]
then
    # - application on server
    python worker.py [server_address] 0 1 [api_secret] $HOSTNAME ./configs/server/config_$2.json
elif [[ $1 == "local" ]]
then
    # - developement on localhost
    python worker.py localhost 0 1 [api_secret] $HOSTNAME ./configs/local/config_$2.json
else
